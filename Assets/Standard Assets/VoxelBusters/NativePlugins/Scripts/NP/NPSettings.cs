...
namespace VoxelBusters.NativePlugins
{
	...
	public class NPSettings : SharedScriptableObject<NPSettings>, IAssetStoreProduct
	{
		...
		private 	const 	string 		kProductVersion					= "1.5.6p2";
...
#pragma warning restore
        ...
        private void RefreshProguardSetup()
        {
#if !(UNITY_WEBPLAYER || UNITY_WEBGL || NETFX_CORE)

            bool _isEnabled = ApplicationSettings.Android.EnableProguardSetup;
            DirectoryInfo _directoryToSearch = new DirectoryInfo(Constants.kAndroidPluginsRootPath);
            FileInfo[] filesInDir = _directoryToSearch.GetFiles("build.gradle*", SearchOption.AllDirectories);

            foreach (FileInfo _file in filesInDir)
            {
                if (_file.Extension == ".meta")
                    continue;
                
                string _dirPath = _file.Directory.FullName + "/";

                if (_isEnabled)
                {
                    FileOperations.Rename(_dirPath + "build.gradle.DISABLED", "build.gradle");
                    FileOperations.Rename(_dirPath + "build.gradle.DISABLED.meta", "build.gradle.meta");
                }
                else
                {
                    FileOperations.Rename(_dirPath + "build.gradle", "build.gradle.DISABLED");
                    FileOperations.Rename(_dirPath + "build.gradle.meta", "build.gradle.DISABLED.meta");
                }
            }
